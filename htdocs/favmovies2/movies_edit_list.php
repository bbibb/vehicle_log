...
$query = 'SELECT * from movielist ORDER BY movieTitle ASC'; // Notice ASC for accending order
...
<table class="table table-striped table-hover"> <!-- Used by Bootstrap to make table responsive to small screens; refer to getbootstrap.com -->
		...
		<th>Date Modified</th>
		<th>&nbsp;</th>
		<!-- <th>&nbsp;</th> DON'T NEED THIS SINCE DELETE IS MOVED TO ADMIN -->
            </tr>
        </thead>
        <tbody>		
...
if (($movie['dateEntered'] == "0000-00-00 00:00:00")) {
        ...
        $dateEntered = date("m-d-Y", strtotime($movie['dateEntered']));
}

if (($movie['dateModified'] == "0000-00-00 00:00:00")) {
        $dateModified = " ";
        } else {
        $dateModified = date("m-d-Y", strtotime($movie['dateModified']));
}
...
?>    
		...
		<!-- <td><?php include "./movies_delete_button.php"; ?><td> DON'T NEED THIS SINCE RECORD DELETE IS MOVED TO THE ADMIN SECTION -->
          </tr>
<?php endforeach; ?>
        </tbody>
    </table>
